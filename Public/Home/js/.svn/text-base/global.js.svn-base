...
{
  var h = $(window).height();
  $("#maincontent").css("min-height",h-63-40);
  $("#nav li").hover(
...
/*
  ...
  $(".searchtxt").click(function(){
  		if($(".searchtxt").val() == "")
  		{
  			$("div#search").find("div.none").show().find('.key').show();
  		}
  })
  /*
...
//search page
	$("#to_configure").click(function(e){
		...
		if(values != "")
		{
			...
			$("form#search_result_form").submit();
		}
	})
});
...
function onLabelClicked()
{
	...
	else
	{
		$.get(configure_addr,{type : type},function(data){
            ...
            if(form.attr('class') == 'bandwidth')
            {
                console.log(form.find('#probe-chooser').find('input[name="pid[]"]').length);
                form.find('#probe-chooser').find('input[name="pid[]"]').each(function(){$(this).attr('checked',false);
                });
                console.log('bandwidth');
                //var li = $('#probelist').clone();
                //li.addClass('added');
                //form.find("#probe-chooser").after(li);
                //li.show();
                form.find('input[name="pid[]"]').click(function(){
                    
                    console.log("input change");
                    if($("select[name='targetprobe']").hasClass('show'))
                    {
                        var targetProbe = $("select[name='targetprobe']");
                       var remains = form.find("#probe-chooser").find('input[name="pid[]"]').not('input:checked');
                       console.log('remains:'+remains.length);
                       if(remains.length <= 0 )
                       {
                           targetProbe.html('').removeClass('show').parent().hide();
                            $("input[name='targettype'][value='probe']").removeAttr('checked');
                            $("input[name='targettype'][value='server']").attr('checked','checked');
                           alert("you shuld choose more probes");
                       }
                       else
                       {
                           targetProbe.html("");
                           remains.each(function(){
                                targetProbe.append("<option value='"+$(this).val()+"'>"+$(this).parent().text()+"</option>");   
                            });
                       }
                    }
                    
                })
                form.find("input[name='targettype']").change(function(){
                    if($(this).val() == "probe")
                    {
                        var probeNum = $('#probelist').find('label').length;
                        var selectNum = form.find('#probe-chooser').find('input[name="pid[]"]:checked').length;
                        console.log(probeNum);
                        console.log(selectNum);
                        if(probeNum <2 || probeNum-selectNum<1)
                        {
                            $("input[name='targettype'][value='probe']").removeAttr('checked');
                            $("input[name='targettype'][value='server']").attr('checked','checked');
                            alert ("You need to  choose more probes");
                        }
                        else
                        {

                           var targetprobe = $("select[name='targetprobe']");
                            form.find('#probe-chooser').find('input[name="pid[]"]').not('input:checked').each(function(){
                                    targetprobe.append('<option value="'+$(this).val()+'">'+$(this).parent().text()+'</option>');                                    
                                });
                            targetprobe.addClass("show").parent().show();
...
$('#submitbtn').click(function()
{
    ...
    $("input[name='dest']").each(function(){
            $(this).val(
                $(this).parent().find("#target-selector option").map(function(){return $(this).text()}).get().join(' ').trim()
                )
...
function onsavebtnclick(){
	console.log('save  btn clicked');
	 var addbtn = $(this).parent().parent().find('.addbtn');
...
function oncancelbtnclick(){
	...
	var buttonline = $(this).parent().parent().find('#button_line');
	txtarea.val('');
	txtarea.hide();
	buttonline.hide();
...
function showAddDialog()
{
 ...
 //$("#add_dest").show();
    ...
    var select = $(this).parent().parent().find('#target-selector');
	var buttonline = $(this).parent().parent().find('#button_line');
    console.log(text);
    console.log(select);
    ...
    else
    {
        ...
        for(var x in values)
        {
            if(values[x] && values[x]!='')
                select.append("<option>"+values[x]+'</option>');
        }
        text.val('');
    }
	buttonline.toggle();
