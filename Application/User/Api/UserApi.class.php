...
class UserApi extends Api{
    ...
    public function register($username, $password, $email, $mobile = ''){
        $id = $this->model->register($username, $password, $email, $mobile);
        $UcenterMember = M('UcenterMember');
        $nMember = $UcenterMember->where(array('id'=>$id))->find();
        $member = array('uid'=>$id,'nickname'=>$nMember['username'],'status'=>1);
        $Member = M('Member');
		if($Member->where(array('uid'=>$id))->count() != 0)
		    return $id;
